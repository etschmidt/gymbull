...
task :mire => :environment do
	bot_id = [10, 12, 6, 35, 41, 61, 49, 63, 68, 
						56, 58, 46, 67, 4, 82, 80, 38, 86, 
						83, 33, 85, 98, 101, 103, 108, 118,
						124, 128, 131].sample
	
	user = User.find_by(id: bot_id)

	post = Post.where.not(user_id: bot_id).sample
	
	mire = Favorite.new(favorited: post, user: user)	
	mire.save
end